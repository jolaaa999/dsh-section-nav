...
- **始终显示** — 章节栏不会折叠成标记条：条目始终保留标题，宽度随聊天区旁的空间伸缩，窗口变窄时逐级收窄。
- **双段悬浮卡片** — 悬浮条目时，主行显示用户的提问，下方小字显示该回合模型最终回答的两行摘要，样式取自宿主的 tooltip token。
- **跳转不被抢回** — 加载更早历史时不再把视口从你刚点击的条目上拖走；分页结束后章节栏会把视口送回原目标。
...
2. 章节栏始终显示：条目展示标题，宽度随消息列旁的空间自动伸缩。
3. 点击章节名称即可跳转。
4. 点击章节右侧星标可收藏/取消收藏。
...
- 视口非常窄时章节栏收窄到最小宽度并固定在视口边缘，标题仍可见但更早被截断。
