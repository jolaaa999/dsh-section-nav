		...
		function startSectionNav(ctx) {
			...
			*/
			...
			const historyProgressSnapshot = () => {
				...
				const identity = (element) => element?.getAttribute("data-chat-anchor-key") ?? element?.getAttribute("data-chat-flow-key") ?? "";
				return `${rows.length}|${identity(first)}|${identity(last)}`;
			};
			const findLoadEarlierControl = () => {
				for (const element of document.querySelectorAll("button, [role=\"button\"], a")) if (LOAD_EARLIER_LABELS.has(normalizeText(element.textContent ?? ""))) return element;
				return null;
			};
			...
			const loadAllHistory = async () => {
				...
				for (let page = 0; page < MAX_HISTORY_PAGES; page += 1) {
					...
					control.click();
					if (!await waitForHistoryProgress(before, 8e3)) return;
					if (destroyed || generation !== historyLoadGeneration) return;
