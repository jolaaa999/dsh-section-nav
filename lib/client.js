window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function pinnedMiniPosition(viewportWidth) {
		...
		*/
		function startSectionNav(ctx) {
			...
			let railPosition = pinnedMiniPosition(document.documentElement.clientWidth || window.innerWidth);
			...
			*/
			...
			const loadAllHistory = async () => {
				if (destroyed || historyLoadInFlight) return;
				historyLoadInFlight = true;
				const generation = ++historyLoadGeneration;
				try {
					for (let page = 0; page < MAX_HISTORY_PAGES; page += 1) {
						if (destroyed || generation !== historyLoadGeneration) return;
						...
						if (control === null) {
							if (!await loadOlderViaSession()) return;
						...
						const before = historyProgressSnapshot();
						control.click();
						if (!await waitForHistoryProgress(before, 1e4)) {
							if (!await loadOlderViaSession()) return;
						}
						updateActiveSections();
						await delay(80);
			...
			const resetForConversation = (nextConversationKey) => {
				...
				drawerOpen = false;
				railPosition = pinnedMiniPosition(document.documentElement.clientWidth || window.innerWidth);
