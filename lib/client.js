		...
		*/
		function createDshAdapter(options = {}) {
			return {
				...
				getAssistantMessages() {
					...
					return uniqueElements(Array.from(container.querySelectorAll(SELECTORS.assistantRow)).filter((element) => !element.hasAttribute("hidden")));
				},
				...
				getMessageByTurnIndex(turnIndex) {
					return Array.from(document.querySelectorAll(`[${ATTRIBUTES.turn}="${String(turnIndex)}"]`)).filter((row) => row.getAttribute(ATTRIBUTES.flowKind) === "assistant-step").at(-1) ?? null;
				},
		...
		var AnswerTracker = class {
			...
			evaluate() {
				...
				if (bestCandidate.score > 0 && (currentHasLeftReadingBand || candidateClearlyWins)) this.setActiveAnswer(bestCandidate);
			}
		...
		function getPreferredModes(viewportWidth) {
			if (viewportWidth >= FULL_BREAKPOINT) return [
				FULL_MODE,
				COMPACT_MODE,
				MINI_MODE
			];
			if (viewportWidth >= COMPACT_BREAKPOINT) return [COMPACT_MODE, MINI_MODE];
			return [MINI_MODE];
		}
		...
		function SectionRail({ activeSectionId, bookmarkedSectionKeys, bookmarkCount, drawerOpen, onSectionSelect, onToggleBookmark, onToggleDrawer, position, sections, t }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("nav", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: sections.map((section) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SectionRailItem, {
		...
		*/
		function startSectionNav(ctx) {
			...
			const updateActiveSections = () => {
				if (!activeAnswer?.element.isConnected) return;
				...
				cacheBookmarkTargets(sections);
				positionManager.setTarget(adapter.getMessageContent(activeAnswer.element) ?? activeAnswer.element);
			...
			answerTracker = new AnswerTracker(adapter, { onActiveAnswerChange(nextActiveAnswer) {
				...
				sections = activeAnswer ? parseSections(activeAnswer.element, adapter, activeAnswer.index) : [];
				cacheBookmarkTargets(sections);
				positionManager.setTarget(activeAnswer ? adapter.getMessageContent(activeAnswer.element) ?? activeAnswer.element : null);
				sectionTracker.setSections(sections);
				render();
			} });
